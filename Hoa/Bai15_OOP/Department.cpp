...
bool Department :: checkID(string id)
{
    ...
    {   
        if ((nullptr != student) && (student->getID() == id)) /* Check x nullptr */
        {
            found = true;
            break;
        }
...
void Department :: addStudent()
{
    ...
    do
    {
        ...
        cout << "2. Add a in-service student" << endl;
...
void Department :: inserviceList()
{
    ...
    for (const auto &student : studentList)
    {
        ...
        if ((nullptr != student) && (INSERVICE == student->isRegular()))
        {
            shared_ptr<InserviceStudent> inserviceStu = dynamic_pointer_cast<InserviceStudent>(student);
...
void Department :: lastest_8List()
{
    for (const auto &student : studentList)
    {
        if ((nullptr != student) && (student->gpaLastest() >= GOOD))
        {
            student->showStudent();
        }
    }
...
void Department :: gpaHighest()
{
    ...
    for (const auto &student : studentList)
    {
        if ((nullptr != student) && (student->gpaMax() > tmpGpaMax))
        {
            tmpGpaMax = student->gpaMax();
        }
    }
